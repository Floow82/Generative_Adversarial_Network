...
class Generator (nn.Module):
  def __init__(self, frf_length, input_size):
    ...
    self.model = nn.Sequential(
        ...
        GBlock(64, 32, kernel_size = 8),
        GBlock(32, 16, kernel_size = 8),
        GBlock(16, 8, kernel_size = 8),
        GBlock(8, 1, kernel_size = 8),
        nn.Linear(self._get_length(), 3001),
        nn.Tanh())
  ...
  def forward (self, coordinates):
    x = self.model(coordinates)
    return x

  def _get_length(self):                                                 #Get the length of the output of the last layer
...
class Discriminator (nn.Module):
  def __init__(self):
    ...
    self.linear = nn.Sequential(
        DBlock(1, 32, kernel_size = 32),
        DBlock(32, 64, kernel_size = 32),
        DBlock(64, 128, kernel_size = 32),
        nn.conv1d(128, 1, kernel_size = 8),
        nn.Sigmoid(),
    )
    #Initialization of layer weights for faster, more stable convergence during the training
    self._initialize_weights()
